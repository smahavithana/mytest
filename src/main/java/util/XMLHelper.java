...
public class XMLHelper {
    ...
    private static String write_file_name = "MyCrawler/Data/";
    private static String read_file_name = "MyCrawler/Data/jobs_20170219_0948.xml";

    public static void writeFromArray(String site, Job[] b) throws Exception {
        ...
        write_file_name = write_file_name + site + "_";
    ...
    public static void writeFromArrayList(String site, ArrayList<Job> myList) throws Exception {
        ...
        for(Job temp:myList){
            ...
            Element title = doc.createElement("title");
            title.appendChild(doc.createTextNode(temp.getTitle()));
            job.appendChild(title);

            Element url = doc.createElement("url");
            url.appendChild(doc.createTextNode(temp.getUrl()));
            job.appendChild(url);

            Element location = doc.createElement("location");
            location.appendChild(doc.createTextNode(temp.getLocation()));
            job.appendChild(location);

            Element date_posted = doc.createElement("datePosted");
            date_posted.appendChild(doc.createTextNode(temp.getDate_posted()));
            job.appendChild(date_posted);

            Element date_expire = doc.createElement("dateExpire");
            date_expire.appendChild(doc.createTextNode(temp.getDate_expire()));
            job.appendChild(date_expire);

            Element date_app_close = doc.createElement("dateAppClose");
            date_app_close.appendChild(doc.createTextNode(temp.getDate_app_close()));
            job.appendChild(date_app_close);

            Element decription = doc.createElement("decription");
            decription.appendChild(doc.createTextNode(temp.getDecription()));
            job.appendChild(decription);

            Element salary = doc.createElement("salary");
            salary.appendChild(doc.createTextNode(temp.getSalary()));
            job.appendChild(salary);

            Element company_name = doc.createElement("companyName");
            company_name.appendChild(doc.createTextNode(temp.getCompany_name()));
            job.appendChild(company_name);

            Element company_logo = doc.createElement("companyLogo");
            company_logo.appendChild(doc.createTextNode(temp.getCompany_logo()));
            job.appendChild(company_logo);

            Element specialism = doc.createElement("specialism");
            specialism.appendChild(doc.createTextNode(temp.getSpecialism()));
            job.appendChild(specialism);
            ...
            Element longDescription = doc.createElement("longDescription");
            longDescription.appendChild(doc.createTextNode(temp.getLong_description()));
            job.appendChild(longDescription);
        ...
        System.out.format("File saved with %d jobs! %n",i);
    }
        ...
        try {
            ...
            for (int temp = 0; temp < size; temp++) {
                ...
                if (nNode.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    j.setLong_description(CWJobs_bak.getJobDescription(eElement.getElementsByTagName("url").item(0).getTextContent()));
            ...
            writeFromArray("site_name", myJobs);
